...
import { listSubjects, saveSubjects } from '../../src/main/services/subjectService'
...
describe('工種科目マスター', () => {
  ...
  it('科目名を変えても明細は改名した科目に付いたまま、新規作成した同名科目は空になる', () => {
    ...
    expect(db.select().from(schema.mDetails).where(eqSubject(target.id)).all()).toHaveLength(1)
    expect(
      db
        .select()
        .from(schema.mDetails)
  ...
  it('明細の無い科目は削除できる', () => {
    ...
    expect(saved.subjects.some((subject) => subject.id === before[0].id)).toBe(false)
