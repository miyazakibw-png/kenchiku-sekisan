...
import { listEstimateRows, saveEstimateRows } from '../../src/main/services/estimateRowService'
import {
  getDeductionLimit,
  getRoomSheet,
  saveDeductionLimit,
...
function roomRow(part3: string, ceilingHeight: number | null): EstimateRowDraft {
  return {
    id: null,
    rowType: 'room',
    part1: '1階',
...
describe('部屋計算書（上段）', () => {
  ...
  it('保存すると天井高さが部位別入力表にも反映される（相互連動）', () => {
    ...
    const [row] = saveEstimateRows(db, { projectId: project.id, rows: [roomRow('事務室', 2.5)] })
    const sheet = getRoomSheet(db, row.id)

    saveRoomSheet(db, {
      id: sheet.id,
      shapeJson: '{"edges":[{"id":"a","direction":"E","length":3,"kind":"wall"}]}',
      ceilingHeight: 3.05,
      note: ''
    })
  ...
  it('物件をコピーすると計算書も独立して複製される', () => {
    ...
    const [row] = saveEstimateRows(db, { projectId: project.id, rows: [roomRow('会議室', 2.4)] })
    const sheet = getRoomSheet(db, row.id)
    saveRoomSheet(db, {
      id: sheet.id,
      shapeJson: '{"edges":[{"id":"a","direction":"E","length":4,"kind":"wall"}]}',
      ceilingHeight: 2.4,
      note: ''
    })
    ...
    // コピー先を直してもコピー元は変わらない
    saveRoomSheet(db, { ...copiedSheet, shapeJson: '{"edges":[]}', ceilingHeight: 2.9 })
  ...
  it('部位別入力表の行を消すと計算書も消える', () => {
    ...
    const [row] = saveEstimateRows(db, { projectId: project.id, rows: [roomRow('倉庫', 2.4)] })
