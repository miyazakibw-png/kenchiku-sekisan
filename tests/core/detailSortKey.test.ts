...
import { sortDetails, type SortableDetail } from '../../src/core/sort/detailSortKey'
...
describe('共通ソートキー', () => {
  it('科目→部位Ⅰ→部位番号→明細番号の優先順で並ぶ', () => {
    ...
    expect(order(rows).map((r) => [r.subjectOrder, r.partNumber, r.detailNumber])).toEqual([
      [1, 1, 2],
      [1, 1, 10],
      [1, 2, 1],
      [2, 1, 1]
  ...
  it('単位は単位マスターの番号順、材種区分は最後の判定に使う', () => {
    ...
    expect(order(rows).map((r) => [r.unitOrder, r.materialCategoryOrder])).toEqual([
      [2, 1],
      [2, 2],
      [9, 1]
    ])
  ...
  it('摘要下→摘要上→備考下→備考上の順で判定する', () => {
    const rows = [
      detail({ descriptionLower: 'あ', descriptionUpper: 'い', remarksLower: 'あ' }),
      detail({ descriptionLower: 'あ', descriptionUpper: 'あ', remarksLower: 'い' }),
      detail({ descriptionLower: 'あ', descriptionUpper: 'あ', remarksLower: 'あ', remarksUpper: 'い' }),
      detail({ descriptionLower: 'あ', descriptionUpper: 'あ', remarksLower: 'あ' })
    ]
    expect(
      order(rows).map((r) => [r.descriptionUpper, r.remarksLower, r.remarksUpper])
    ).toEqual([
      ['あ', 'あ', ''],
      ['あ', 'あ', 'い'],
      ['あ', 'い', ''],
