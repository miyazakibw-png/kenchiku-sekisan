...
describe("建具表の自動計算", () => {
  ...
  it("腰高がある建具は巾木を差し引かない", () => {
    const calc = computeFitting(
      row({ width: 1.5, height: 1.76, sillHeight: 0.9 }),
    );
  ...
  it("計算式が不正なときは知らせる", () => {
    const calc = computeFitting(
      row({ width: 2, height: 2, areaFormula: "W*" }),
    );
  ...
  it("軸組横補強：腰高なしは W", () => {
    expect(computeFitting(row({ width: 1.2, height: 2 })).reinforcement).toBe(
      1.2,
    );
  });
  ...
  it("軸組横補強：腰高ありは W*2", () => {
    expect(
      computeFitting(row({ width: 1.5, height: 1.2, sillHeight: 0.9 }))
        .reinforcement,
    ).toBe(3);
  });
  ...
  it("軸組横補強：腰高が無くても巾木減を変えたら W*2-巾木減", () => {
    const calc = computeFitting(
      row({ width: 1.8, height: 2, baseboardFormula: "1.8-0.3" }),
    );
...
describe("建具記号", () => {
  it("重複している行が分かる", () => {
    expect([...duplicateSymbolIndexes(["AW1", "AW2", "AW1", ""])]).toEqual([
      0, 2,
    ]);
  });
