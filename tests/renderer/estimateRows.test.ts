...
describe('部位Ⅰ・部位Ⅱの引き継ぎ', () => {
  ...
  it('部位Ⅱ別仕訳も部位Ⅱと一緒に引き継ぐ', () => {
    const rows = [row({ part2: '1階', part2Split: 1 }), row({ part3: '事務室' })]
...
describe('Excelからの貼り付け', () => {
  ...
  it('マスターに無い型枠の文字も入力できる', () => {
    const preview = buildPastePreview([emptyRow()], columns, '\t\t\t特殊型枠', 0, 0, () =>
      emptyRow()
    )
  ...
  it('天井高さに数値以外を貼るとエラーになる', () => {
    const preview = buildPastePreview([emptyRow()], columns, '\t\t\t\t\tあいう', 0, 0, () =>
      emptyRow()
    )
