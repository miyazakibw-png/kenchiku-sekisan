...
function project(id: number, name: string, projectDate: string): ProjectSummary {
  return {
    id,
...
describe('物件管理台帳の並べ替え', () => {
  ...
  it('列を指定して並べ替えられる', () => {
    expect(sortProjects(projects, 'projectDate').map((p) => p.id)).toEqual([2, 1, 3])
    expect(sortProjects(projects, 'name', true).map((p) => p.id)).toEqual([3, 1, 2])
  })
})
