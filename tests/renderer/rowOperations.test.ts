...
describe("明細マスターの行操作", () => {
  ...
  it("明細番号は数値のみ受け付け、不正入力を拒否する", () => {
    ...
    expect(updateDetailNumberInput(rows, 0, "302.")[0].detailNumberInput).toBe(
      "302.",
    );
  ...
  it("フォーカスアウトで明細番号を小数2桁へ整形する", () => {
    ...
    expect(normalizeDetailNumberInput(edited, 0)[0].detailNumberInput).toBe(
      "302.50",
    );
  });
});
...
describe("履歴からの復元（保存を挟むケース）", () => {
  ...
  it("進むで戻ってきた行は削除対象から外れる", () => {
    ...
    const next = restoreSnapshot(current, {
      rows: [kept, restored],
      deletedIds: [],
    });
