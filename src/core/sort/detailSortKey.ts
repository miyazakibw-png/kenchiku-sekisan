...
export type SortSegment =
  { kind: "number"; value: number | null } | { kind: "text"; value: string };
...
export function buildSortKey(detail: SortableDetail): SortSegment[] {
  const hasPart2 =
    detail.part2Name.trim() !== "" || detail.part2SortOrder !== null;
  return [
 ...
 */
export function sortDetails<T>(
  rows: T[],
  select: (row: T) => SortableDetail,
): T[] {
  return rows
