...
const AXIS: Record<EdgeDirection, 'x' | 'y'> = { E: 'x', W: 'x', N: 'y', S: 'y' }
...
export function rectangleShape(width: number, depth: number): RoomShape {
  return {
    edges: [edge('E', width), edge('S', depth), edge('W', width), edge('N', depth)]
  }
}
...
export function solveShape(shape: RoomShape): SolvedShape {
  ...
  for (const axis of ['x', 'y'] as const) {
    ...
    const total = axisEdges.reduce(
      (sum, row) => (row.length === null ? sum : sum + SIGN[row.direction] * row.length),
      0
    )

    if (blanks.length === 0) {
      if (round2(total) !== 0) {
        error = axis === 'x' ? '横方向の寸法が閉じていません' : '縦方向の寸法が閉じていません'
      }
      continue
    }
    if (blanks.length > 1) {
...
export function roomQuantities(solved: SolvedShape, ceilingHeight: number | null): RoomQuantities {
  ...
  return {
    ...
    baseboardLength: round2(totals.wall + totals.column),
    wallArea: height === null ? null : round2(totals.wall * height),
 ...
 */
export function roomSymbols(solved: SolvedShape, ceilingHeight: number | null): RoomSymbol[] {
  const quantities = roomQuantities(solved, ceilingHeight)
  ...
  for (const row of solved.edges) {
    ...
    if (row.kind === 'wall') {
      ...
      symbols.push({
        ...
        value: ceilingHeight === null ? null : round2(row.resolved * ceilingHeight),
        edgeId: row.id
      })
...
export function splitEdge(shape: RoomShape, id: string, firstLength: number): RoomShape {
  const index = shape.edges.findIndex((row) => row.id === id)
  if (index < 0) return shape
  const target = shape.edges[index]
  const rest = target.length === null ? null : round2(target.length - firstLength)
  return {
    edges: [
...
export function updateEdge(shape: RoomShape, id: string, patch: Partial<RoomEdge>): RoomShape {
  return { edges: shape.edges.map((row) => (row.id === id ? { ...row, ...patch } : row)) }
}

/** 取り合いの欠除：この面積以下は差し引かない（設定画面で変更する） */
export const DEFAULT_DEDUCTION_LIMIT = 0.5

export function deducts(area: number, limit = DEFAULT_DEDUCTION_LIMIT): boolean {
  return area > limit
}
