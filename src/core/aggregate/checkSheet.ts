...
function partIdOf(item: CheckSheetSourceItem, parts: readonly CheckSheetPart[]): number | null {
  const byNumber = aggregationPartIdOf(item.partNumber)
  if (byNumber !== null && parts.some((part) => part.id === byNumber)) return byNumber
...
export function buildCheckSheet(
  ...
  const targets = items.filter((item) => item.materialCategory === materialCategory)
  ...
  targets.forEach((item) => {
    ...
    const block = blocks.get(blockKey) ?? new Map<number, Map<string, number>>()
...
export function toCheckSheetTsv(sheet: CheckSheet): string {
  ...
  sheet.blocks.forEach((block, index) => {
    ...
    for (let row = 0; row < rowCount; row += 1) {
      const cells = [row === 0 ? block.part1 : '', row === 0 ? block.part2 : '', '']
      block.columns.forEach((column) => {
        const cell = column[row]
        cells.push(cell ? cell.name : '', '', cell ? cell.quantity.toFixed(2) : '', '')
      })
