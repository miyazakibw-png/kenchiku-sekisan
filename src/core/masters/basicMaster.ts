...
export function isBlankBasicMasterRow(row: BasicMasterRow): boolean {
  return (
    (!Number.isInteger(row.id) || row.id < 1) &&
    row.name.trim() === "" &&
    row.note.trim() === ""
  );
}
...
/** 何も入っていない行を除いた並び */
export function dropBlankBasicMasterRows(
  rows: readonly BasicMasterRow[],
): BasicMasterRow[] {
 ...
 */
export function validateBasicMaster(
  ...
  if (filled.length > limit.maxRows) {
    errors.push(
      `${limit.label}は${limit.maxRows}件までです（${filled.length}件）`,
    );
  }
  ...
  filled.forEach((row) => {
    if (!Number.isInteger(row.id) || row.id < 1 || row.id > limit.maxId) {
      errors.push(
        `番号は1〜${limit.maxId}で入力してください（${row.id || "空欄"}）`,
      );
...
export function nextBasicMasterId(
  kind: BasicMasterKind,
  rows: readonly BasicMasterRow[],
): number {
