...
export function computeFitting(row: FittingInput): FittingComputed {
  ...
  const areaByFormula = row.areaFormula.trim()
    ? evaluateFormula(row.areaFormula, vars)
    : null;
  const areaFormulaError =
    row.areaFormula.trim() !== "" && areaByFormula === null;
  const area =
    areaByFormula !== null
  ...
  const baseByFormula = row.baseboardFormula.trim()
    ...
    : null;
  const baseboardFormulaError =
    row.baseboardFormula.trim() !== "" && baseByFormula === null;
...
export function expandSymbols(series: SymbolSeries): string[] {
  ...
  for (let n = Math.trunc(from); n <= Math.trunc(to); n++)
