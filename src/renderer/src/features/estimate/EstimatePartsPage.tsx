...
import type { EstimateRowDraft, MasterOptions, ProjectSummary } from '@shared/types'
...
export default function EstimatePartsPage({ project, options, onBack }: Props): JSX.Element {
  ...
  const save = useCallback(async () => {
    const saved = await window.sekisan.saveEstimateRows({ projectId: project.id, rows })
  ...
  const pasteFromExcel = useCallback(async () => {
    ...
    if (!text.trim()) return
    const preview = buildPastePreview(rows, columns, text, selected, 0, () => emptyRow())
  ...
  const copyFromOtherProject = useCallback(async (source: ProjectSummary) => {
    ...
    setClipboard(copied)
    setOthers(null)
    setMessage(`${source.managementNo} から ${copied.length} 行を控えました（行貼り込みで挿入）`)
  }, [])
  ...
  return (
    <div className="estimate-page">
      <div className="toolbar">
        ...
        <button
          type="button"
          onClick={() => setRows([...rows.slice(0, selected + 1), subtotalRow(), ...rows.slice(selected + 1)])}
        >
          Σ 小計行
        </button>
      ...
      <div className="check-setting">
        ...
        <select value={checkCategory} onChange={(e) => setCheckCategory(e.target.value)}>
        ...
        <span className="note">
          床・巾木・壁・柱・梁・張天井・廻り縁は、その部屋の計算書で拾った{checkCategory}
          を表示します（計算書の作成後に反映）。
        </span>
      </div>
      ...
      {others && (
        <div className="other-projects">
          <span>コピー元の物件</span>
          {others.map((row) => (
            <button key={row.id} type="button" onClick={() => void copyFromOtherProject(row)}>
      ...
      <MasterCodeOptions entries={options.formworkCategories} listId="formwork-list" />
          ...
          {rows.map((row, index) => {
            ...
            return (
              <tr
                ...
                className={
                  [index === selected ? 'selected' : '', isSubtotal ? 'subtotal' : '']
                    .filter(Boolean)
                    .join(' ') || undefined
                }
              ...
              >
                ...
                <td>
                  <input
                    ...
                    onChange={(e) => setRows(updateRow(rows, index, { part1: e.target.value }))}
                  />
                </td>
                <td>
                  <input
                    ...
                    title="空欄のときは入力のある上の行を引き継ぎます"
                    onChange={(e) => setRows(updateRow(rows, index, { part2: e.target.value }))}
                  />
                </td>
                <td className="flag">
                  <input
                    ...
                    title="集計時に部位Ⅱ別で仕分ける"
                    onChange={(e) =>
                      setRows(updateRow(rows, index, { part2Split: e.target.checked ? 1 : 0 }))
                    }
                  />
                </td>
                <td>
                  <MasterCodeInput
                    ...
                    onChange={(value) => setRows(updateRow(rows, index, { formwork: value }))}
                  />
                </td>
                <td>
                  <input
                    value={row.part3}
                    onChange={(e) => setRows(updateRow(rows, index, { part3: e.target.value }))}
                  />
                </td>
                <td>
                  {isSubtotal
                    ? ''
                    : numberCell(index, row.ceilingHeight, 2, parseNumber, (value) => ({
                        ceilingHeight: value
                      }))}
                </td>
                <td>
                  {numberCell(index, row.multiplier, 0, parseMultiplier, (value) => ({
                    multiplier: value ?? 1
                  }))}
                </td>
                <td>
                  {isSubtotal ? (
                    ...
                    <select
                      value={row.calcType}
                      onChange={(e) =>
                        setRows(updateRow(rows, index, { calcType: e.target.value }))
                      }
                    >
