...
/** 控えた行を新しい行として複製する（IDは持っていかない） */
function clonedRows(copied: TransferRowDraft[]): TransferRowDraft[] {
  return copied.map((row) => ({ ...row, id: null }));
}

/** 上書貼付（カーソルの行から、控えた行数だけ置き換える） */
export function overwriteTransferRows(
  rows: TransferRowDraft[],
  index: number,
  copied: TransferRowDraft[],
): TransferRowDraft[] {
  const at = Math.min(Math.max(index, 0), rows.length);
  const cloned = clonedRows(copied);
  return [...rows.slice(0, at), ...cloned, ...rows.slice(at + cloned.length)];
}

/** 挿入貼付（カーソルの行の上へ差し込み、元の行は下へずらす） */
export function insertTransferRows(
  rows: TransferRowDraft[],
  index: number,
  copied: TransferRowDraft[],
): TransferRowDraft[] {
  const at = Math.min(Math.max(index, 0), rows.length);
  return [...rows.slice(0, at), ...clonedRows(copied), ...rows.slice(at)];
}

/** 追加貼付（最終行の下へ足す） */
export function appendTransferRows(
  rows: TransferRowDraft[],
  copied: TransferRowDraft[],
): TransferRowDraft[] {
  return [...rows, ...clonedRows(copied)];
}
