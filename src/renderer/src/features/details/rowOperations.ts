...
export function updateDetailNumberInput(
  ...
  return rows.map((row, i) =>
    i === index
      ? {
          ...row,
          detailNumberInput: input,
          detailNumber: parseDetailNumber(input),
        }
      : row,
  );
}
...
export function normalizeDetailNumberInput(
  rows: DraftRow[],
  index: number,
): DraftRow[] {
  if (index < 0 || index >= rows.length) return rows;
  ...
  return rows.map((row, i) =>
    i === index ? { ...row, detailNumberInput: formatted } : row,
  );
}
