...
export function buildFittingColumns(): GridColumn<FittingDraft>[] {
  const numberColumn = (
    ...
    set: (row: FittingDraft, value: number | null) => FittingDraft
    ...
    set: (row, text) => {
      ...
      return parsed.error ? { row, error: parsed.error } : { row: set(row, parsed.value) }
    }
  })
  ...
  return [
    ...
    numberColumn(
      ...
      'W',
      (row) => row.width,
      (row, value) => ({ ...row, width: value })
    ),
    numberColumn(
      'height',
      'H',
      (row) => row.height,
      (row, value) => ({ ...row, height: value })
    ),
    numberColumn(
      'sillHeight',
      '腰高',
      (row) => row.sillHeight,
      (row, value) => ({ ...row, sillHeight: value })
    ),
