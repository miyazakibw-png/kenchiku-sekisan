...
import { evaluateFormula, normalizeFormula } from '../../../../core/formula/evaluate'
...
export function parseNumber(text: string): { value: number | null; error?: string } {
  const trimmed = text.trim().replace(/[０-９．]/g, (c) => String.fromCharCode(c.charCodeAt(0) - 0xfee0))
 ...
 */
export function parseSize(text: string): { value: number | null; formula: string; error?: string } {
  ...
  if (value === null || !Number.isFinite(value)) {
    return { value: null, formula: '', error: '数値か計算式（例 900+150）で入力してください' }
  }
  return { value: Math.round(value * 100) / 100, formula: normalizeFormula(trimmed) }
}
...
export function sortBySymbol(rows: FittingDraft[]): FittingDraft[] {
  return [...rows].sort((a, b) => {
    if (a.fromEstimate !== b.fromEstimate) return a.fromEstimate - b.fromEstimate
    return compareSymbols(a.symbol, b.symbol)
  })
}

/** 自動計算列。貼り付け位置を画面と合わせるために列は持つが、値は取り込まない */
function calculatedColumn(key: string, label: string): GridColumn<FittingDraft> {
  return {
    key,
    label,
    get: () => '',
    set: (row, value) =>
      value.trim() === '' ? { row } : { row, warning: `${label}は自動計算のため取り込みません` }
  }
}
...
export function buildFittingColumns(): GridColumn<FittingDraft>[] {
  const numberColumn = (
    ...
    set: (row: FittingDraft, value: number | null, formula: string) => FittingDraft
  ...
  return [
    ...
    numberColumn(
      ...
      'W',
      (row) => row.width,
      (row, value, formula) => ({ ...row, width: value, widthFormula: formula })
    ),
    numberColumn(
      'height',
      'H',
      (row) => row.height,
      (row, value, formula) => ({ ...row, height: value, heightFormula: formula })
    ),
    numberColumn(
      'sillHeight',
      '腰高',
      (row) => row.sillHeight,
      (row, value, formula) => ({ ...row, sillHeight: value, sillHeightFormula: formula })
    ),
    ...
    },
    {
      ...
      set: (row, value) => ({ row: { ...row, baseboardFormula: value.trim() } })
    },
    {
      key: 'note',
      label: 'その他（備考）',
