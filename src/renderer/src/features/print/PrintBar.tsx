...
/** 印刷前に、全幅がA3横1枚に収まる縮小率を決めておく */
function applyScale(area: HTMLElement): void {
  const table = area.querySelector('table')
  const width = Math.max(area.scrollWidth, table?.scrollWidth ?? 0)
  area.style.setProperty('--print-scale', String(printScale(width)))
}
...
export default function PrintBar({ projectName }: { projectName: string }): JSX.Element {
  ...
  return (
    <div className="print-bar">
      <button
        type="button"
        title="今の画面をA3横1枚の幅に収めて印刷します"
        disabled={busy}
        onClick={() => void run(async () => void (await window.sekisan.printPaper()))}
      >
        🖨 印刷
      </button>
      <button
        type="button"
        title="今の画面をA3横のPDFで保存します"
        disabled={busy}
        onClick={() =>
          void run(async (_area, name) => void (await window.sekisan.printPdf(name)))
        }
      >
        📄 PDF
      </button>
