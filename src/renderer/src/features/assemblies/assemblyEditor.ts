...
export function updateNumberInput(
  ...
  if (index < 0 || index >= items.length || !isValidDetailNumberInput(input))
    return items;
  const inputField =
    field === "partNumber" ? "partNumberInput" : "detailNumberInput";
  return items.map((item, i) =>
    i === index
      ? { ...item, [field]: parseDetailNumber(input), [inputField]: input }
      : item,
  );
}
...
export function updateCoefficientInput(
  ...
  const coefficient =
    input === "" || Number.isNaN(parsed) ? items[index].coefficient : parsed;
  return items.map((item, i) =>
...
function toSortable(
  ...
  return {
    ...
    materialCategoryOrder:
      materialCategories.find((c) => c.name === item.materialCategory)
        ?.displayOrder ?? null,
  };
}
...
export function sortAssemblies(
  ...
  return sortDetails(assemblies, (assembly) => {
    ...
    return head
      ? toSortable(head, subjectOrderById, units, materialCategories)
      : empty;
  });
}
