...
import { buildPastePreview, copyRangeAsTsv, type GridColumn } from '../grid/gridClipboard'
...
export default function BasicMasterPage(): JSX.Element {
  ...
  const paste = useCallback(async (): Promise<void> => {
    ...
    if (!text.trim()) return
    const preview = buildPastePreview(rows, columns, text, selected, selectedCol, () => ({
      id: 0,
      name: '',
      note: ''
    }))
  ...
  return (
    ...
    >
      ...
      <div className="hint">
        ...
        ／Enter・↑↓でセル移動、Ctrl+VでExcelから貼り付け
      </div>
      {messages.length > 0 && (
