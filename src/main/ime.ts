...
async function loadApi(): Promise<Win32Api | null> {
  ...
  try {
    ...
    api = {
      ...
      getWindow: user32.func("__stdcall", "GetWindow", "uintptr", [
        "uintptr",
        "uint",
      ]),
      ...
      immGetDefaultIMEWnd: imm32.func(
        "__stdcall",
        "ImmGetDefaultIMEWnd",
        "uintptr",
        ["uintptr"],
      ),
      immGetContext: imm32.func("__stdcall", "ImmGetContext", "uintptr", [
        "uintptr",
      ]),
      ...
      immSetOpenStatus: imm32.func("__stdcall", "ImmSetOpenStatus", "int", [
        "uintptr",
        "int",
      ]),
      immGetConversionStatus: imm32.func(
        "__stdcall",
        "ImmGetConversionStatus",
        "int",
        [
      ...
      immSetConversionStatus: imm32.func(
        "__stdcall",
        "ImmSetConversionStatus",
        "int",
 ...
 */
export async function setImeMode(
  ...
  for (const handle of windowHandles(win32, top)) {
    ...
    if (context !== 0n) {
      ...
      if (read !== 0)
        hiragana = HIRAGANA_MODE | (conversion[0] & IME_CMODE_ROMAN);
      const keep =
        read !== 0 && sentence[0] !== 0 ? sentence[0] : IME_SMODE_PHRASEPREDICT;
      const opened = win32.immSetOpenStatus(context, open ? 1 : 0);
      let converted = 0;
      if (open)
        converted = win32.immSetConversionStatus(context, hiragana, keep);
      win32.immReleaseContext(handle, context);
      report.push(
        `窓${handle}: 入力状態=${opened} 変換=${converted} 文=${keep}`,
      );
    }
    ...
    if (open)
      win32.sendMessage(ime, WM_IME_CONTROL, IMC_SETCONVERSIONMODE, hiragana);
    report.push(`窓${handle}: IMEの窓=${ime}`);
  }

  if (report.length === 1)
    report.push("日本語入力の入り口が見つかりませんでした");
  return report.join(" / ");
}
