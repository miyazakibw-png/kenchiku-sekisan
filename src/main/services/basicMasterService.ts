...
export function listBasicMasters(db: AppDatabase): BasicMasters {
  return {
    pickupParts: db
      .select({
        id: mPickupParts.id,
        name: mPickupParts.name,
        note: mPickupParts.note,
      })
      .from(mPickupParts)
    ...
    materialCategories: db
      ...
      .orderBy(
        asc(mMaterialCategories.displayOrder),
        asc(mMaterialCategories.id),
      )
      .all()
      .map((row) => ({ ...row, note: "" })),
    units: db
    ...
    formworkCategories: db
      ...
      .orderBy(
        asc(mFormworkCategories.displayOrder),
        asc(mFormworkCategories.id),
      )
      .all()
      .map((row) => ({ ...row, note: "" })),
  };
  ...
  db.transaction((tx) => {
    if (kind === "pickupParts") {
      tx.select({ id: mPickupParts.id })
        ...
        .forEach((row) =>
          tx.delete(mPickupParts).where(eq(mPickupParts.id, row.id)).run(),
        );
    ...
    if (kind === "materialCategories") {
      tx.select({ id: mMaterialCategories.id })
        ...
        .forEach((row) =>
          tx
            .delete(mMaterialCategories)
            .where(eq(mMaterialCategories.id, row.id))
            .run(),
        );
      trimmed.forEach((row, index) => {
    ...
    if (kind === "aggregationParts") {
      tx.select({ id: mAggregationParts.id })
        ...
        .forEach((row) =>
          tx
            .delete(mAggregationParts)
            .where(eq(mAggregationParts.id, row.id))
            .run(),
        );
      trimmed.forEach((row, index) => {
    ...
    tx.select({ id: mFormworkCategories.id })
      ...
      .forEach((row) =>
        tx
          .delete(mFormworkCategories)
          .where(eq(mFormworkCategories.id, row.id))
          .run(),
      );
    trimmed.forEach((row, index) => {
