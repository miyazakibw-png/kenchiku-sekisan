...
import type { SaveSubjectsResult, Subject, SubjectDraft } from '../../shared/types'
...
export function listSubjects(db: AppDatabase): Subject[] {
  return db.select().from(mSubjects).orderBy(asc(mSubjects.displayOrder), asc(mSubjects.id)).all()
}
 ...
 */
export function saveSubjects(db: AppDatabase, rows: SubjectDraft[]): SaveSubjectsResult {
  const blocked: { id: number; name: string }[] = []
  db.transaction((tx) => {
    const existing = tx.select().from(mSubjects).all()
    const keptIds = new Set(rows.map((row) => row.id).filter((id): id is number => id !== null))
    ...
    blocked.forEach((subject, offset) => {
      tx.update(mSubjects)
        .set({ code: subjectCode(rows.length + offset), displayOrder: rows.length + offset })
        .where(eq(mSubjects.id, subject.id))
        .run()
    })
  })
  return { subjects: listSubjects(db), blockedDeletes: blocked.map((subject) => subject.name) }
}
