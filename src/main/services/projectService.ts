...
export function listProjectLedger(db: AppDatabase): ProjectLedger {
  const rows = db
    ...
    .all();
  return {
    projects: rows.map((row) => toSummary(db, row)),
    fields: listProjectFields(db),
  };
}
...
export function createProject(db: AppDatabase, name: string): ProjectSummary {
  ...
  // 科目・部位・材種区分・単位・型枠分類も工事専用として複製する
  copyBasicMastersToProject(db, id);
...
function remapLayout(
  ...
  try {
    ...
    const mapped = placements.map((placement: { estimateRowId?: number }) => {
      ...
      return copiedRowId === undefined
        ? placement
        : { ...placement, estimateRowId: copiedRowId };
    });
...
export function copyProject(
  ...
  return db.transaction((tx) => {
    const source = tx
      ...
      .get();
    if (!source)
      throw new Error(`コピー元の物件が見つかりません (id=${sourceId})`);
    ...
    // 工事専用マスター（科目・部位・材種区分・単位・型枠分類）も引き継ぐ
    tx.select()
      .from(projectMasters)
      .where(eq(projectMasters.projectId, sourceId))
      .all()
    ...
    tx.select()
      ...
      .all()
      .forEach(
        ({ id: _id, projectId: _projectId, finishAssemblyId, ...rest }) => {
...
export function saveProject(
  ...
  return db.transaction((tx) => {
    ...
    const fieldIds = new Set(
      listProjectFields(tx as AppDatabase).map((field) => field.id),
    );
...
export function saveProjectFields(
  db: AppDatabase,
  fields: ProjectField[],
): ProjectField[] {
  db.transaction((tx) => {
    ...
    fields.forEach((field, index) => {
      ...
      if (field.id > 0) {
        tx.update(mProjectFields)
          .set(values)
          .where(eq(mProjectFields.id, field.id))
          .run();
      } else {
